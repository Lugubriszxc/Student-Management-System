...
namespace Student_Management_System
{
    public partial class Form2 : Form
    {
        ...
        private void Form2_Load(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT StudInfo.StudentID, StudInfo.FirstName+ ' ' +StudInfo.MiddleName+ ' '+StudInfo.LastName AS Name, SubDepartment.DegreeProgram, SubDepartment.DepartmentName FROM SubDepartment INNER JOIN StudInfo ON SubDepartment.StudentID = StudInfo.StudentID", con);
            DataTable dt = new DataTable();
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (dialogresult == DialogResult.Yes)
            {
                ...
                SqlCommand cmde = new SqlCommand("SELECT StudInfo.StudentID, StudInfo.FirstName+ ' ' +StudInfo.MiddleName+ ' '+StudInfo.LastName AS Name, SubDepartment.DegreeProgram, SubDepartment.DepartmentName FROM SubDepartment INNER JOIN StudInfo ON SubDepartment.StudentID = StudInfo.StudentID", con);
        ...
        {
            using (IDbConnection con = new SqlConnection("Data Source=LAPTOP-K946DC8Q\\SQLEXPRESS;Initial Catalog=DBLOGIN;Integrated Security=True"))
            {
                con.Open();
                string query = "SELECT StudInfo.StudentID, StudInfo.FirstName + ' ' + StudInfo.MiddleName + ' ' + StudInfo.LastName AS Name, StudInfo.AddBarangay + ', ' + StudInfo.AddCity + ', ' + StudInfo.AddProvince AS Address, SubDepartment.section, SubDepartment.DepartmentName, SubDepartment.DegreeProgram, SubDepartment.YearLevel, Subdepartment.Semester, Grades.Subject, Grades.Unit, Grades.Instructor, Grades.Grade, Grades.TotalUnit, Grades.GWA" +
                " FROM((StudInfo INNER JOIN SubDepartment ON StudInfo.StudentID = SubDepartment.StudentID)" +
                " INNER JOIN Grades ON StudInfo.StudentID = Grades.StudentID) " +
                " WHERE StudInfo.StudentID = '" + StudentID + "'";

                studInfoBindingSource.DataSource = con.Query<StudInfo>(query, commandType: CommandType.Text);
                StudInfo obj = studInfoBindingSource.Current as StudInfo;
                List<StudInfo> list = con.Query<StudInfo>(query, commandType: CommandType.Text).ToList();
